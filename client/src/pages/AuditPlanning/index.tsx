...
const AuditPlanning: React.FC<AuditPlanningProps> = ({
  ...
  // When embedded inside AuditDetail, use local state to avoid clobbering ?tab=
  const [localStep, setLocalStep] = useState<StepKey>(STEP_CONFIG[0].key);
  const currentStep: StepKey = embedded
    ? localStep
    : (searchParams.get("step") as StepKey) || "entity";
