...
const ProcedureWorkspace: React.FC<{
  ...
  // Use the document source selected in the Analytical Review step of Audit Planning.
  // Falls back to TB if no selection has been made yet.
  const arDocSource = store.auditDocSources?.[auditId] ?? "tb";

  // Derive the LGA code prefix (e.g. "MU") to match codes shown in Analytical Review
  const _audit = store.audits?.find((a) => a.id === auditId);
  const _lgaName = _audit
    ? (store.lgas?.find((l) => l.id === _audit.lgaId)?.name ?? "")
    : "";
  const lgaCodePrefix = _lgaName
    ? _lgaName.replace(/\s.*/, "").slice(0, 2).toUpperCase()
    : "";
  const realTb = store.trialBalances?.find((tb) => tb.auditId === auditId);
  const fmtCode = (raw: string | null | undefined) =>
    raw ? (lgaCodePrefix ? `${lgaCodePrefix}-${raw}` : raw) : "";
  const tbLines: {
    id: string;
    code: string;
    account: string;
